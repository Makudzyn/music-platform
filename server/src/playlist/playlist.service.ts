...
export class PlaylistService {
  ...
  private async calculatePlaylistStats(
    ...
    const totalDuration = tracks.reduce(
      (sum, track: Track) => sum + (track.duration || 0),
      0,
    );
  ...
  private async findPlaylistById(
    playlistId: mongoose.Types.ObjectId,
  ...
  async createPlaylist(
    ...
    const artist = await this.artistModel
      .findById(createPlaylistDto.artist)
      .exec();
    if (!artist) {
      throw new NotFoundException(
        `Artist with ID ${createPlaylistDto.artist} not found`,
      );
    }
  ...
  async editPlaylistInfo(
    playlistId: mongoose.Types.ObjectId,
    editInfoDto: EditInfoDto,
  ...
  async removeTracks(
    ...
    existingPlaylist.tracks = existingPlaylist.tracks.filter(
