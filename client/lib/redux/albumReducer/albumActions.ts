...
export const loadAlbumById = createAsyncThunk<
  Playlist,
  string,
  { state: RootState }
>('albums/loadAlbumById', async (albumId: string, { getState }) => {
  const state = getState();
  const existingAlbum = state.albums.albums.find(
...
// Use the basic function to create thunk
