...
export default function ArtistView({ artistId, scrollRef }: ArtistViewProps) {
  ...
  const { artist, loading: artistLoading } =
    useAppSelector(selectArtistViewData);
  const { tracks, loading: tracksLoading } = useAppSelector(
    selectTracksByArtistId,
  );
  ...
  return (
    <>
      {artistLoading ? (
        ...
        <ArtistHeader artist={artist} tracks={tracks} />
      ) : (
      ...
      <CarouselSection
        items={albums}
        renderItem={albumsLoading ? renderSkeleton : renderAlbum}
      />
      <h2 className="font-bold text-lg leading-6 text-foreground mt-3 mb-1">
        Featured songs
      </h2>
