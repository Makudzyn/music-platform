...
export default function PlaylistCard({playlist}: PlaylistCardProps) {
  const capitalizedType = playlist.type.charAt(0).toUpperCase() + playlist.type.slice(1);
      ...
      <CardContent className="pb-4 px-0">
        <Link href={`/playlist/${playlist._id}`} className="group/title">
          <div className="aspect-square relative overflow-hidden rounded-t-md mb-4">
            ...
            />
            <div
              className="absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"/>
          </div>
        ...
        <p className="text-sm text-muted-foreground truncate px-4">
          ...
          <Link
            ...
            href={playlist.type === 'album' ? `/artists/${playlist.artist._id}` : `/users/${playlist.owner._id}`}
          >
            {playlist.type === 'album' ? playlist.artist.name : playlist.owner.username}
          </Link>
        </p>
      </CardContent>
    </Card>
